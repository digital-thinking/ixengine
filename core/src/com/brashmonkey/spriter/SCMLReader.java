...
public class SCMLReader {
    ...
    protected void loadTimelineKeys(ArrayList<Element> keys, Timeline timeline) {
        for (int i = 0; i < keys.size(); i++) {
            ...
            Timeline.Key.BoneObject boneObject;
            if (obj.getName().equals("bone"))
                boneObject = new Timeline.Key.BoneObject(position, scale, pivot, angle, alpha, new FileReference(folder, file));
            else
                boneObject = new Timeline.Key.BoneObject(position, scale, pivot, angle, alpha, new FileReference(folder, file));
            key.setBoneObject(boneObject);
            timeline.addKey(key);
        }
    }
