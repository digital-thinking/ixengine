...
public class Animation {
    ...
    protected void update(BoneRef ref, Bone root, int time) {
        ...
        if (isObject)
            this.tweenObject((BoneObject) bone1, (BoneObject) bone2, (BoneObject) tweenTarget, t, key.curve, key.spin);
    ...
    void unmapTimelineObject(int timeline, boolean isObject, Bone root) {
        ...
        if (isObject) ((BoneObject) mapTarget).set((BoneObject) tweenTarget);
    ...
    protected void tweenObject(BoneObject boneObject1, BoneObject boneObject2, BoneObject target, float t, Curve curve, int spin) {
        this.tweenBone(boneObject1, boneObject2, target, t, curve, spin);
        target.alpha = curve.tweenAngle(boneObject1.alpha, boneObject2.alpha, t);
        target.ref.set(boneObject1.ref);
    }
    ...
    public void prepare() {
        ...
        for (int i = 0; i < this.tweenedKeys.length; i++) {
            ...
            this.tweenedKeys[i].setBoneObject(new BoneObject(new Point(0, 0)));
            this.unmappedTweenedKeys[i].setBoneObject(new BoneObject(new Point(0, 0)));
        }
