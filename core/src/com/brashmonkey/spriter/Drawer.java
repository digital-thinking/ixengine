 ...
 * To draw a {@link Player} call {@link #draw(Player)}. This method relies on {@link #draw(BoneObject)}, which has to be implemented with the chosen backend.
...
public abstract class Drawer<R> {
     ...
     */
    public void drawObjectBoxes(Player player, Iterator<BoneObject> it) {
        while (it.hasNext()) {
            BoneObject bone = it.next();
            this.drawBox(player.getBox(bone));
        }
    }
     ...
     */
    public void drawPoints(Player player, Iterator<BoneObject> it) {
        while (it.hasNext()) {
            BoneObject point = it.next();
     ...
     * @param map the character map to draw
     */
    public void draw(Iterator<BoneObject> it, CharacterMap[] maps) {
        while (it.hasNext()) {
            BoneObject boneObject = it.next();
            if (boneObject.ref.hasFile()) {
                if (maps != null) {
                    for (CharacterMap map : maps)
                        if (map != null)
                            boneObject.ref.set(map.get(boneObject.ref));
                }
                this.draw(boneObject);
            }
        }
    }
     ...
     *
     * @param boneObject the object to draw.
     */
    public abstract void draw(BoneObject boneObject);
}
