...
public class Player {
    ...
    private final HashMap<BoneObject, Timeline.Key> objToTimeline = new HashMap<BoneObject, Timeline.Key>();
    public int speed;
     ...
     */
    public BoneObject getObject(int index) {
        return (BoneObject) this.unmappedTweenedKeys[getCurrentKey().getObjectRef(index).timeline].object();
    }
     ...
     */
    public BoneObject getObject(String name) {
        return (BoneObject) this.unmappedTweenedKeys[animation.getTimeline(name).id].object();
    }

    /**
     * Returns a object reference for the given time line bone.
     *
     * @param boneObject the time line object
     * @return the object reference for the given bone
     * @throws NullPointerException if no reference for the given object was found
     */
    public ObjectRef getObjectRef(BoneObject boneObject) {
        return this.getCurrentKey().getObjectRefTimeline(this.objToTimeline.get(boneObject).id);
    }
    ...
    public void setObject(String name, float x, float y, float angle, float scaleX, float scaleY, float pivotX, float pivotY, float alpha, int folder, int file) {
        ...
        BoneObject boneObject = getObject(index);
        boneObject.set(x, y, angle, scaleX, scaleY, pivotX, pivotY, alpha, folder, file);
        unmapObjects(ref);
    }
     ...
     *
     * @param name       the name of the object
     * @param boneObject the object with the new values
     * @throws SpriterException if no object exists of the given name
     */
    public void setObject(String name, BoneObject boneObject) {
        setObject(name, boneObject.position, boneObject.angle, boneObject.scale, boneObject.pivot, boneObject.alpha, boneObject.ref);
    }
    ...
    public void setEntity(Entity entity) {
        ...
        for (int i = 0; i < maxAnims; i++) {
            ...
            key.setBoneObject(new BoneObject(new Point(0, 0)));
            keyU.setBoneObject(new BoneObject(new Point(0, 0)));
            tweenedKeys[i] = key;
     ...
     */
    public Iterator<BoneObject> objectIterator() {
     ...
     * @return the object iterator
     */
    public Iterator<BoneObject> objectIterator(ObjectRef start) {
     ...
     * @author Trixt0r
     */
    class ObjectIterator implements Iterator<BoneObject> {
        int index = 0;
        ...
        @Override
        public Timeline.Key.BoneObject next() {
