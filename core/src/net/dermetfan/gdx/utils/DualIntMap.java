...
public class DualIntMap<V> {
     ...
     */
    public DualIntMap() {
        keyToValue = new IntMap<V>();
        valueToKey = new ObjectIntMap<V>();
    }
    ...
    public DualIntMap(int initialCapacity) {
        keyToValue = new IntMap<V>(initialCapacity);
        valueToKey = new ObjectIntMap<V>(initialCapacity);
    }
    ...
    public DualIntMap(int initialCapacity, float loadFactor) {
        keyToValue = new IntMap<V>(initialCapacity, loadFactor);
        valueToKey = new ObjectIntMap<V>(initialCapacity, loadFactor);
    }
    ...
    public DualIntMap(IntMap<V> map) {
        keyToValue = new IntMap<V>(map);
        valueToKey = new ObjectIntMap<V>(map.size);
        Keys keys = map.keys();
        while (keys.hasNext) {
            int key = keys.next();
    ...
    public DualIntMap(DualIntMap<V> map) {
        keyToValue = new IntMap<V>(map.keyToValue);
        valueToKey = new ObjectIntMap<V>(map.valueToKey);
    }
