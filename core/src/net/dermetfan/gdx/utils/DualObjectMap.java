...
public class DualObjectMap<K, V> {
    ...
    public DualObjectMap() {
        keyToValue = new ObjectMap<K, V>();
        valueToKey = new ObjectMap<V, K>();
    }
    ...
    public DualObjectMap(int initialCapacity) {
        keyToValue = new ObjectMap<K, V>(initialCapacity);
        valueToKey = new ObjectMap<V, K>(initialCapacity);
    }
    ...
    public DualObjectMap(int initialCapacity, float loadFactor) {
        keyToValue = new ObjectMap<K, V>(initialCapacity, loadFactor);
        valueToKey = new ObjectMap<V, K>(initialCapacity, loadFactor);
    }
    ...
    public DualObjectMap(ObjectMap<K, V> map) {
        keyToValue = new ObjectMap<K, V>(map);
        valueToKey = new ObjectMap<V, K>(map.size);
        for (K key : map.keys())
    ...
    public DualObjectMap(DualObjectMap<K, V> map) {
        keyToValue = new ObjectMap<K, V>(map.keyToValue);
        valueToKey = new ObjectMap<V, K>(map.valueToKey);
    }
